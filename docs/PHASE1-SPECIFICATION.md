...
## 7. Open Questions for Developer (must be answered before Phase 2)

1. **LLM provider(s)** — which model/provider should Phase 2 target first (affects Token Manager rate cards and Cost Tracker)? Anthropic Claude is assumed as the default per the global CLAUDE.md guidance to default to the latest Claude models, but confirm before hardcoding.
2. **Background job backend** — does the target Redmine install already run Sidekiq/Resque/Delayed Job, or should Phase 2 assume plain ActiveJob with the default async adapter for local dev only?
3. **MCP transport** — in-process tool calls only, or does this need to expose an actual MCP server endpoint for external agent clients (e.g. Claude Desktop, other IDEs) per the "RedmineFlux MCP server" mentioned in `redmineflux_devops/CLAUDE.md`? If a shared MCP server already exists for DevOps, AgentOS tools should likely register into that same server rather than standing up a second one.
4. **Confirmation UX for irreversible actions (AD-5)** — modal in the Agent Dashboard, or a dedicated "Pending Approvals" queue?
5. **Code-writing agents (AD-2)** — confirmed out of scope for the initial roadmap, or should Phase 1 already reserve schema/permission space for a future `manage_agent_code_writes` permission?

---

## 8. Phase Roadmap (unchanged from the brief, restated for tracking)

| Phase | Deliverable | Status |
|---|---|---|
| Phase 1 | Functional spec, architecture, folder structure, DB schema, navigation, permissions, agent lifecycle, UI wireframes | **This document set — awaiting approval** |
| Phase 2 | Plugin skeleton (`init.rb`, empty modules, routes, permission registration) | Blocked on Phase 1 approval |
| Phase 3 | Database migrations | Blocked on Phase 2 |
| Phase 4+ | Incremental module implementation, one module at a time, each behind its own SDD task + three-gate review | Blocked on Phase 3 |
